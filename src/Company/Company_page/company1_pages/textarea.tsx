...
interface User {
  ...
  complaint:
    | string
    | { COMPLAINT: string; _id: string; email: string; name: string };
  rows: string;
  textarea: string;
}

interface CompanyTextareaProps {
  handleSendFeedbackButtonClick: (user: User, textareaData: string) => void;
}

export default function CompanyTextarea() {
  ...
  const handleSendFeedbackButtonClick = (users: User[]) => {
    users.forEach((user) => {
      const formData = {
      ...
      fetch("http://195.35.22.190:5000/submit_feedback", {
        method: "POST",
        headers: {
  ...
  return (
    <form>
      ...
      <button
        ...
        onClick={() => handleSendFeedbackButtonClick(users)}
      >
        send feedback
      </button>
    </form>
